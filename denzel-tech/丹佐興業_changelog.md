...
### v2.7（06-24）補上作圖規格速查表
- Tony 抓到：v2.5 下載本機化的圖（背景圖＋3個合作夥伴LOGO）是原圖尺寸直接存檔，沒有照 leon-day／龍德的慣例先給「網站實際需要的畫布尺寸」讓 Tony 照規格後製裁切
- 補上「作圖規格速查」表：檢查後 3 個合作夥伴 LOGO 內容乾淨不需重裁；`denzel-stack-bg.jpg` 是原始直式 768×1024，建議 Tony 後製裁一張橫式 1920×1080 版本給 Hero／Footer 用，目前直式圖靠瀏覽器 cover 自動裁切暫時可接受
...
## 作圖規格速查（給 Tony 後製裁切用，仿 leon-day／龍德「七、作圖規格速查」）

> Tony 抓到的問題：丹佐這批圖（denzel-stack-bg／3 個合作夥伴 LOGO）是直接從官網／截圖抓下來的原圖尺寸，沒有像 leon-day／龍德那樣先給「網站實際需要的畫布尺寸」讓 Tony 照規格裁切修圖。以下補上規格，之後新素材一律先給這張表再下載/裁切。

| 素材 | 建議畫布尺寸 | 格式 | 用途 | 目前狀態 |
|---|---|---|---|---|
| Hero／Footer 共用底圖 | **1920×1080（16:9，橫式）** | JPG | `.hero-bg-img`／`.footer-bg-img`，`background-size:cover` | ⚠️ 目前 `denzel-stack-bg.jpg` 是原始 768×1024 **直式**圖，cover 模式下桌機寬螢幕會自動裁切置中，目前看起來尚可，但建議 Tony 用同張照片重新裁一張橫式 16:9 版本，避免裁切位置被瀏覽器決定 |
| 合作夥伴 LOGO ×3 | 不強制統一比例（LOGO 類，CSS 固定 `height:44px / width:auto`，等比縮放不變形） | PNG（透明底）優先，JPG 次之 | `.partner-card-logo` | ✅ 已檢查三張圖內容乾淨（HomeLink 白底／電小二＋冠宇透明底），不需重新裁切 |
| LOGO 正式版 | 待 Tony 提供，建議 SVG 或至少 1000px 以上 PNG 透明底 | SVG 優先 | Hero／QR彈窗／Footer／favicon＋og-image 來源圖 | ⏳ 待提供 |
| Favicon | 512×512（1:1） | PNG | 瀏覽器分頁圖示 | ✅ 已有，LOGO 換正式版後要重新產生 |
| OG 分享圖 | 1200×630（1.91:1） | JPG | LINE／FB 分享預覽 | ✅ 已有，LOGO 換版＋文案待同步更新後要重新產生 |
| 案例照 ×10（§03／§04 共用） | 沿用各圖既有尺寸即可（見素材清單 D 區） | JPG | §03 輪播＋§04 底圖 | ✅ 已有，不用重新處理 |
